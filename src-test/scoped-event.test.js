describe("scopedEvent", function() {
  ...
  describe("trigger", function() {
    it("calls all listeners bound to the same event type & scope", function() {
      var callCount = 0;
      oInstance.bind('evtype:this.that.other', function() {
        callCount++;
      });
    ...
    it("calls all listeners when an event is triggered at an enclosing scope", function() {
      console.log('>>>');
      var callCount = 0;
      oInstance.bind('evtype:this.that.*', function() {
        callCount++;
      });
      oInstance.bind('evtype:this.*', function() {
        callCount++;
      });
      oInstance.bind('evtype:*', function() {
        callCount++;
      });
      oInstance.bind('*', function() {
        callCount++;
      });
      oInstance.trigger('evtype:this.that.other');
